...
export default {
  ...
  computed: {
    ...mapState({
      categoryList: ({ category }) => category.list,
    }),
    ...mapGetters({
  ...
  methods: {
    ...mapActions(["getCategoryList", "deleteCategory"]),
    ...
    initData() {
      this.getCategoryList(this.filterOptions);
            ...
            handler: () => {
              this.deleteCategory(item._id)
                .then((res) => {
                  ...
                  const index = this.categoryList.indexOf(item, 0);
                  if (index > -1) {
