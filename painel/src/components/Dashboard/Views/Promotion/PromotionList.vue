...
<script>
  ...
  const headers = ["Titulo","Data Cadastrado"];
  const tableColumns = [ 
    "title", 
    {
  ...
  export default {
    ...
    computed: {
      ...mapState({
        promotionList: ({promotion}) => promotion.list
      }),
      ...mapGetters({
    ...
    methods: {
      ...mapActions(["getPromotionList", "deletePromotion"]),
        ...
        buttons: [
          {
            ...
            handler: () => { 
                ...
                const index = this.promotionList.indexOf(item, 0);
                  if (index > -1) {
